...
class ActivationService implements IActivationService
{
  ...
  {
    $this->baseUrl = 'http://localhost/'; // Your API base URL
  }
  ...
  public function getActivationInfo(?string $activationCode = null): Collection
  {
    ...
    $apiUrl = $this->baseUrl . 'wp-json/license-activation/v1/activation-status?activation_code=' . $activationCode;
  ...
  public function activate(string $licenseKey): Collection
  {
    try{
      ...
      $request = ['purchase_code' => $licenseKey, 'domain' => $serverInfo['domainUrl'],'activation_type' => 'live', 'server_info' => $serverInfo, 'item_id' => config('variables.itemId')];
  ...
  public function envatoActivate(
    ...
    $apiUrl = $this->baseUrl . 'api/v1/core/activation/envato-activate';
    ...
    $requestData = [
      ...
      'domain'            => $domain,
      'email'             => $email,
      'activation_type'   => in_array($activationType, ['live', 'localhost']) ? $activationType : 'live',
      'server_info'       => wp_json_encode($serverInfo),
    ];
  ...
  public function checkValidActivation(?string $licenseKey = null): bool
  {
    try {
      ...
      $apiUrl = $this->baseUrl . 'wp-json/license-activation/v1/validate-activation?activation_code=' . $licenseKey;
